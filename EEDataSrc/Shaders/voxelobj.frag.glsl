...
const float ATLAS_SCALAR = 0.0625f;
...
void main() {
    ...
    for (int i = 0; i < lightCount; i++) {
		...
		if (i == 0) {
			float depth = (lightspacePos.z / lightspacePos.w) - 0.0001;
	...
	FragColor = vec4(lo * texture(albedoSampler, (inUV + vec2(inTexID)) * ATLAS_SCALAR).rgb, 1.0);
