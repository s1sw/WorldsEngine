...
namespace ImGui
{
    ...
    IMGUI_API bool          Begin(const char8_t* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
    IMGUI_API void          End();
    ...
    IMGUI_API void          TextUnformatted(const char8_t* text, const char8_t* text_end = NULL);
	...
	IMGUI_API bool          CollapsingHeader(const char8_t* label, ImGuiTreeNodeFlags flags = 0);  // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
...
struct ImGuiIO
{
    ...
    ImVec2      DisplayOffset;                  // = (0, 0)         // Offset for the main display
