...
namespace worlds {
    RenderMeshManager::RenderMeshManager(R2::VK::Core* core)
        : core(core) {
    ...
    RenderMeshInfo& RenderMeshManager::loadOrGet(AssetID asset) {
        ...
        size_t indicesSize = lmd.indexType == IndexType::Uint16 ? lmd.indices16.size() * sizeof(uint16_t) : lmd.indices32.size() * sizeof(uint32_t);

        RenderMeshInfo meshInfo{};
        meshInfo.indexOffset = indexBuffer->Allocate(indicesSize, meshInfo.indexAllocationHandle);
        meshInfo.vertsOffset = vertexBuffer->Allocate(lmd.vertices.size() * sizeof(Vertex), meshInfo.vertexAllocationHandle);
        ...
        core->QueueBufferUpload(vertexBuffer->GetBuffer(), lmd.vertices.data(), lmd.vertices.size() * sizeof(Vertex), meshInfo.vertsOffset);
