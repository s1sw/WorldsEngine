...
namespace worlds
{
    Result<void *, IOError> LoadFileToBuffer(std::string path, int64_t *fileLength)
    {
        ...
        if (file == nullptr)
        {
            ...
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to open file %s due to following error: %s",
                         path.c_str(), PHYSFS_getErrorByCode(errCode));
        ...
        if (diskFileLength == -1)
        {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to obtain the length of file %s.", path.c_str());
        ...
        {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Only read %lld bytes of %lld in file %s", readBytes,
                         diskFileLength, path.c_str());
    ...
    Result<void *, IOError> loadAssetToBuffer(AssetID asset, int64_t *fileLength)
    {
        ...
        {
            PHYSFS_ErrorCode errCode = PHYSFS_getLastErrorCode();
            logErr("Failed to open file %s due to following error: %s", AssetDB::idToPath(asset).c_str(),
        ...
        if (diskFileLength == -1)
        {
            logErr("Failed to obtain the length of file %s.", AssetDB::idToPath(asset).c_str());
        ...
        if (readBytes != diskFileLength)
        {
            logErr("Only read %lld bytes of %lld in file %s", readBytes, diskFileLength,
