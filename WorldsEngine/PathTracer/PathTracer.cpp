...
namespace worlds {
    ...
    PathTraceResult PathTracer::pathTrace(glm::vec3 pos, glm::vec3 dir, int depth) {
        if (depth > MAX_PT_DEPTH) return PathTraceResult{};
        ...
        if (hitAnything) {
            ...
            reg.view<WorldLight, Transform>().each([&](WorldLight& wl, Transform& t) {
                ...
                if (wl.type == LightType::Directional) {
                    ...
                    result.direct += glm::pow(wl.color, glm::vec3(2.2)) * wl.intensity * (obscured ? 0.0f : 1.0f) * glm::max(glm::dot(norm, -forward), 0.0f) * albedo;
                    ...
                    result.direct += glm::pow(wl.color, glm::vec3(2.2)) * wl.intensity * cosLi * spotScalar * falloffScalar * albedo * shadowFac;
                }
            });
