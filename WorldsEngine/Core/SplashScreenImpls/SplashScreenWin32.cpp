...
namespace
{
    ...
    // WS_THICKFRAME: without this the window cannot be resized and so aero snap, de-maximizing and minimizing won't
    // work WS_SYSMENU: enables the context menu with the move, close, maximize, minize... commands (shift + right-click
    // on the task bar item) WS_CAPTION: enables aero minimize animation/transition WS_MAXIMIZEBOX, WS_MINIMIZEBOX:
...
namespace worlds
{
    ...
    SplashScreenImplWin32::SplashScreenImplWin32(bool small)
    {
        ...
        s->t = new std::thread([this, small] {
            ...
            s->hwnd = CreateWindowEx(0, CLASS_NAME, "Loading...", static_cast<DWORD>(Style::aero_borderless),
                                     clientRect.left, clientRect.top, width, height, nullptr, nullptr, nullptr, this);
    ...
    LRESULT SplashScreenImplWin32::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_NCHITTEST: {
            ...
            return hit_test(hwnd, POINT{GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)});
            break;
        }
        case WM_NCACTIVATE: {
            if (!composition_enabled())
