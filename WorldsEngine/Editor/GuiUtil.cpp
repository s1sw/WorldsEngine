...
namespace worlds
{
    ...
    const robin_hood::unordered_map<std::string, const char8_t*> extensionIcons = {
        ...
        {".png", ICONSTR_TEXTURE}, {".jpg", ICONSTR_TEXTURE},   {".tga", ICONSTR_TEXTURE},
    ...
    void listDirectoryItems(std::string path, std::function<void(std::string, PHYSFS_FileType)> onClick,
            ...
            listDirectoryItems(
                *currentDirectory,
                [&](std::string path, PHYSFS_FileType type) {
                    if (type == PHYSFS_FILETYPE_DIRECTORY)
                             ...
                             const char** fileExtensions, int fileExtensionCount, const char* startingDir)
    {
        ...
        if (ImGui::BeginPopupModal(title, nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove))
        {
            ...
            {
                std::string absPath =
                    rootOffset + (currentDirectory->empty() ? currFile : (*currentDirectory + "/" + currFile));
                PHYSFS_Stat stat;
                PHYSFS_stat(absPath.c_str(), &stat);
                std::string extension = std::filesystem::path(currFile).extension().string();
    ...
    void drawPopupNotifications()
    {
        ...
        for (auto it = popupNotifications.rbegin(); it != popupNotifications.rend(); it++)
        {
            ...
            float alpha = glm::min(glm::min(1.0f, notification.shownFor * 5.0f),
                                   ((popupDuration - 0.5f) - notification.shownFor) * 2.0f);
