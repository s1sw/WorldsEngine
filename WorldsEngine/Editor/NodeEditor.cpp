...
namespace worlds::nodes {
    ...
    void NodeEditor::draw() {
        ...
        if (ImGui::BeginChild("Nodes", size, true, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
            ...
            if (ImGui::IsMousePosValid() && ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows) && ImGui::IsMouseDragging(ImGuiMouseButton_Right)) {
            ...
            for (float x = 0.0f; x < size.x; x += gridSpacing) {
                dl->AddLine(gridOffset + glm::vec2(x, -size.y), gridOffset + glm::vec2(x, size.y * 2.0f), ImColor(1.f, 1.f, 1.f, 0.1f));
            }

            for (float y = 0.0f; y < size.y; y += gridSpacing) {
                dl->AddLine(gridOffset + glm::vec2(-size.x, y), gridOffset + glm::vec2(size.x * 2.0f, y), ImColor(1.f, 1.f, 1.f, 0.1f));
            }
