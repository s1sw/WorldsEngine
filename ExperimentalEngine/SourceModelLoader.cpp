...
#define MAX_NUM_LODS 8
    ...
    struct vertexFileHeader_t {
        ...
        vertexFileFixup_t* getFixupBlock() {
            return (vertexFileFixup_t*)((byte*)this + fixupTableStart);
        }
    };
...
#pragma pack(pop)
    ...
    void loadSourceModel(AssetID mdlId, AssetID vtxId, AssetID vvdId, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, LoadedMeshData& lmd) {
        ...
        memcpy(vertexBlockDup, vertexBlock, sizeof(mstudiovertex_t) * vertCount);

        int target = 0;
        for (int i = 0; i < vvd->numFixups; i++) {
            memcpy(
                vertexBlock + target,
                vertexBlockDup + fixupBlock[i].sourceVertexID,
                fixupBlock[i].numVertexes * sizeof(mstudiovertex_t));

            target += fixupBlock[i].numVertexes;
        }
