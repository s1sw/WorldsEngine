...
static SpvReflectResult ParseDescriptorBlockVariableUsage(
    ...
    case SpvOpTypeStruct:
    {
        ...
        if (index >= p_var->member_count) {
            // DIRTY HACK!!!!
            // This should be SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_BLOCK_MEMBER_REFERENCE,
            // but that seems to cause issues with certain inputs (possibly related to use of structs?).
            // If we just return and stop processing the access chain, everything parses correctly,
