...
namespace vku {
	...
	public:
		...
		explicit UniqueVmaAllocation(VmaAllocation& allocation, VmaAllocator allocator)
			: allocation(allocation)
			, allocator(allocator)
		{}

		UniqueVmaAllocation(UniqueVmaAllocation const&) = delete;

		UniqueVmaAllocation(UniqueVmaAllocation&& other) noexcept
			: allocation(other.release())
			, allocator(other.allocator)
		{}

		UniqueVmaAllocation& operator=(UniqueVmaAllocation&& other) noexcept {
			allocation = other.release();
			allocator = other.allocator;
			return *this;
		}
		...
		~UniqueVmaAllocation() {
			if (allocation) vmaFreeMemory(allocator, allocation);
		}

		VmaAllocation release() {
			VmaAllocation temp = allocation;
			allocation = nullptr;
			return temp;
		}

		VmaAllocation const& operator*() const {
			return allocation;
		}
	...
	public:
		...
		GenericBuffer(vk::Device device, VmaAllocator allocator, vk::BufferUsageFlags usage, vk::DeviceSize size, VmaMemoryUsage memUsage = VMA_MEMORY_USAGE_GPU_ONLY) {
			...
			vmaCreateBuffer(allocator, &cci, &allocInfo, &cBuf, &vmaAlloc, nullptr);
			allocation = UniqueVmaAllocation(vmaAlloc, allocator);
	...
	public:
		...
		UniformBuffer(const vk::Device& device, VmaAllocator allocator, size_t size, VmaMemoryUsage memUsage = VMA_MEMORY_USAGE_GPU_ONLY) : GenericBuffer(device, allocator, vk::BufferUsageFlagBits::eUniformBuffer | vk::BufferUsageFlagBits::eTransferDst, size, memUsage) {
		}
	};
